...
describe('web auth routes', () => {
  ...
  it('renders Sukumad observability route and navigation when permission is granted', async () => {
    ...
    vi.stubGlobal(
      ...
      vi.fn(async (input: RequestInfo | URL) => {
        const url = typeof input === 'string' ? input : input.toString()
        if (url.includes('/observability/workers?')) {
          return new Response(
            JSON.stringify({
              items: [],
              totalCount: 0,
        ...
        if (url.includes('/observability/rate-limits?')) {
          return new Response(
            JSON.stringify({
              items: [],
              totalCount: 0,
  ...
  it('saves RapidPro reporter sync mappings through backend API', async () => {
    ...
    vi.stubGlobal(
      ...
      vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
        ...
        if (url.includes('/settings/rapidpro-reporter-sync/preview?reporterId=11') && (!init?.method || init.method === 'GET')) {
          return new Response(
            JSON.stringify({
              reporter: {
                id: 11,
