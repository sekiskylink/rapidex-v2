...
import { Close as CloseIcon, ReceiptLongRounded as ReceiptLongRoundedIcon, SearchRounded as SearchRoundedIcon } from '@mui/icons-material'
...
export function GlobalRequestFinderDrawer({ open, canReadRequests, onClose }: GlobalRequestFinderDrawerProps) {
  ...
  React.useEffect(() => {
    ...
    setErrorMessage('')
    void apiClient
      .request<PaginatedResponse<RequestSummaryRow>>(`/api/v1/requests?${query.toString()}`)
      .then((response) => {
  ...
  }, [apiClient, canReadRequests, debouncedMsisdn, open, range.from, range.to, selectedOrgUnit?.uid])
  ...
  React.useEffect(() => {
    ...
    void apiClient
      .request<PaginatedResponse<OrgUnitOption>>(`/api/v1/orgunits?page=0&pageSize=20&search=${encodeURIComponent(query)}`)
      .then((response) => {
  ...
  }, [apiClient, canReadRequests, debouncedOrgUnitQuery, open, selectedOrgUnit])
