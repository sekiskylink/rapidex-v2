...
export function OrgUnitsPage() {
  ...
  const load = React.useCallback(async () => {
    ...
    try {
      ...
      const response = await apiClient.request<OrgUnitListResponse>(`/api/v1/orgunits?${query.toString()}`)
      setItems(response.items ?? [])
      const state = await apiClient.request<SyncState>('/api/v1/orgunits/sync-state')
      setSyncState(state)
    } catch (loadError) {
  ...
  const loadLevels = React.useCallback(async () => {
    try {
      const response = await apiClient.request<OrgUnitLevelListResponse>('/api/v1/orgunits/levels')
  ...
  async function submitOrgUnit() {
    ...
    try {
      ...
      await apiClient.request<OrgUnit>(editing ? `/api/v1/orgunits/${editing.id}` : '/api/v1/orgunits', {
  ...
  async function deleteOrgUnit(unit: OrgUnit) {
    setError('')
    try {
      await apiClient.request<void>(`/api/v1/orgunits/${unit.id}`, { method: 'DELETE' })
      await load()
    } catch (deleteError) {
  ...
  async function runSync() {
    ...
    setSyncResult(null)
    try {
      const response = await apiClient.request<SyncResult>('/api/v1/orgunits/sync', {
        method: 'POST',
        body: JSON.stringify({
    ...
    <Box>
      <Stack direction={{ xs: 'column', sm: 'row' }} justifyContent="space-between" spacing={2} sx={{ mb: 2 }}>
        ...
        <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1}>
          {principal?.permissions.includes('orgunits.write') ? (
