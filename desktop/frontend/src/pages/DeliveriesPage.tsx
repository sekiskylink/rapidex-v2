  ...
  const openDetailDialog = async (row: DeliveryRow) => {
    ...
    try {
      const [detail, events] = await Promise.all([
        apiClient.request<DeliveryDetailRecord>(`/api/v1/deliveries/${row.id}`),
        apiClient.request<PaginatedResponse<EventRecord>>(`/api/v1/deliveries/${row.id}/events?page=1&pageSize=50&sort=createdAt:asc`),
      ])
