...
export function DocumentationPage() {
  ...
  React.useEffect(() => {
    ...
    setLoadingList(true)
    setErrorMessage('')
    apiClient.request<DocumentationListResponse>('/api/v1/documentation')
      .then((payload) => {
        if (!active) {
          return
        }
  ...
  React.useEffect(() => {
    ...
    setErrorMessage('')
    apiClient.request<DocumentationDetail>(`/api/v1/documentation/${encodeURIComponent(selectedSlug)}`)
      .then((payload) => {
        if (active) {
          setDocument(payload)
        }
