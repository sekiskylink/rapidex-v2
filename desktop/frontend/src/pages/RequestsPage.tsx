...
export function RequestsPage() {
  ...
  const canWrite = Boolean(principal?.permissions.includes('requests.write'))
  ...
  const loadServers = React.useCallback(async () => {
    ...
    try {
      const response = await apiClient.request<PaginatedResponse<{ id: number; name: string; code: string }>>(
        '/api/v1/servers?page=1&pageSize=200&sort=name:asc',
      )
      setServers(response.items.map((item) => ({ id: item.id, name: item.name, code: item.code })))
    } catch (error) {
      await handleAppError(error, { fallbackMessage: 'Unable to load servers.' })
    } finally {
  ...
  const handleCreate = async () => {
    ...
    setCreateErrorMessage('')
    try {
      await apiClient.request('/api/v1/requests', {
        method: 'POST',
      ...
      await handleAppError(error, { fallbackMessage: 'Unable to create request.' })
    } finally {
      setSubmitting(false)
    }
  }
  ...
  return (
    <Stack spacing={3}>
      ...
      <RequestForm
        ...
        testId="desktop-request-create-form-grid"
        submitLabel="Create"
