...
export function ServersPage() {
  ...
  const canWrite = Boolean(principal?.permissions.includes('servers.write'))
  ...
  const fetchServers = React.useCallback(
    async (params: AppDataGridFetchParams) => {
      ...
      const response = await apiClient.request<PaginatedResponse<ServerRow>>(`/api/v1/servers?${query}`)
      return {
        rows: response.items,
  ...
  const openEditDialog = async (row: ServerRow) => {
    ...
    setEditErrorMessage('')
    try {
      const detail = await apiClient.request<ServerRow>(`/api/v1/servers/${row.id}`)
  ...
  const submitCreate = async () => {
    ...
    setCreateErrorMessage('')
    try {
      await apiClient.request('/api/v1/servers', {
        method: 'POST',
  ...
  const submitEdit = async () => {
    ...
    setEditErrorMessage('')
    try {
      await apiClient.request(`/api/v1/servers/${editServer.id}`, {
        method: 'PUT',
  ...
  const toggleSuspended = async (row: ServerRow) => {
    try {
      await apiClient.request(`/api/v1/servers/${row.id}`, {
        method: 'PUT',
        body: JSON.stringify({
          ...row,
          suspended: !row.suspended,
        ...
        <AdminRowActions
          ...
          actions={[
            ...
            },
            {
              ...
              confirmMessage: params.row.suspended ? `Activate ${params.row.name}?` : `Suspend ${params.row.name}?`,
            },
            {
              id: 'delete',
              label: 'Delete',
  ...
  return (
    <Stack spacing={2.5}>
      ...
      <ServerFormDialog
        ...
        testId="desktop-server-create-form-grid"
        submitLabel="Create"
      ...
      <ServerFormDialog
        ...
        testId="desktop-server-edit-form-grid"
        submitLabel="Save"
        onClose={() => {
          setEditOpen(false)
          setEditServer(null)
