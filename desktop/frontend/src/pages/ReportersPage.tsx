...
export function ReportersPage() {
  ...
  const load = React.useCallback(async () => {
    ...
    try {
      const [reporterResponse, orgUnitResponse, reporterGroupResponse] = await Promise.all([
        ...
        apiClient.request<{ items: ReporterGroupOption[] }>('/api/v1/reporter-groups/options'),
      ])
  ...
  const fetchFacilityOptions = React.useCallback(async (search: string) => {
    const response = await apiClient.request<ListResponse<OrgUnit>>(`/api/v1/orgunits?page=0&pageSize=20&leafOnly=true&search=${encodeURIComponent(search)}`)
