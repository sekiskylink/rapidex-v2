...
func newRouter(deps AppDeps) *gin.Engine {
	...
	if deps.AuthHandler != nil && deps.JWTManager != nil {
		...
		admin.GET("/mine", middleware.RequirePermission(deps.RBACService, rbac.PermissionAPITokensWrite), deps.AuthHandler.ListMyActiveAPITokens)
	...
	if deps.SettingsHandler != nil {
		...
		settingsGroup.POST("/rapidex-webhook-mappings/metadata/refresh", middleware.RequirePermission(deps.RBACService, rbac.PermissionSettingsWrite, middleware.WithAdminRoleOverride()), deps.SettingsHandler.RefreshRapidexWebhookMetadata)
